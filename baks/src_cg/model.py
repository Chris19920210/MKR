...
from layers import Dense, FCUnit, CrossCompressUnit, CycleGANUnit
...
class MKR(object):
    ...
    def _build_low_layers(self, args):
        ...
        self.cg_unit = CycleGANUnit(args.dim)
        ...
        for _ in range(args.L):
            ...
            self.item_embeddings, self.head_embeddings, fake_v, fake_e = self.cg_unit([input_v, input_e])
    ...
    def _build_loss(self, args):
        ...
        self.cg_loss_rs = self.cg_unit.get_rs_loss(self.reals[0], self.fakes[0])
        for inputs, fakes in zip(self.reals[1:], self.fakes[1:]):
            self.cg_loss_rs += self.cg_unit.get_rs_loss(inputs, fakes)
        ...
        self.cg_loss_kge = self.cg_unit.get_kge_loss(self.reals[0], self.fakes[0])
        for inputs, fakes in zip(self.reals[1:], self.fakes[1:]):
            self.cg_loss_kge += self.cg_unit.get_kge_loss(inputs, fakes)
