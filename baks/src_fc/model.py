...
from layers import Dense, FCUnit, CrossCompressUnit, FCGUnit
...
class MKR(object):
    ...
    def _build_low_layers(self, args):
        ...
        fc_unit = CrossCompressUnit(args.dim)
        ...
        for _ in range(args.L):
            ...
            self.item_embeddings, self.head_embeddings = fc_unit([self.item_embeddings, self.head_embeddings])
            self.tail_embeddings = tail_mlp(self.tail_embeddings)

            self.vars_rs.extend(user_mlp.vars)
            self.vars_kge.extend(tail_mlp.vars)

        self.vars_rs.extend(fc_unit.vars)
        self.vars_kge.extend(fc_unit.vars)
