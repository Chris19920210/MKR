...
def train(args, data, show_loss, show_topk):
    ...
    with tf.Session() as sess:
        ...
        for step in range(args.n_epochs):
            ...
            start = 0
            while start < train_data.shape[0]:
                _, loss = model.train(sess, get_feed_dict_for_rskg(model, train_data, start, start + args.batch_size))
                start += args.batch_size
                if show_loss:
                    print(loss)

            # CTR evaluation
            train_auc, train_acc = model.eval(sess, get_feed_dict_for_rskg(model, train_data, 0, train_data.shape[0]))
            eval_auc, eval_acc = model.eval(sess, get_feed_dict_for_rskg(model, eval_data, 0, eval_data.shape[0]))
            test_auc, test_acc = model.eval(sess, get_feed_dict_for_rskg(model, test_data, 0, test_data.shape[0]))
...
def get_feed_dict_for_rskg(model, data, start, end):
    feed_dict = {model.user_indices: data[start:end, 0],
                 model.item_indices: data[start:end, 1],
                 model.labels: data[start:end, 2],
                 model.head_indices: data[start:end, 3],
                 model.relation_indices: data[start:end, 4],
                 model.tail_indices: data[start:end, 5]}
    return feed_dict
